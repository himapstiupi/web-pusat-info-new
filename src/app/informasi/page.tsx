...
export const metadata: Metadata = {
  title: "Pusat Informasi Mahasiswa PSTI",
};
      ...
      <main className="flex-grow flex flex-col w-full">
        ...
        <section className="relative w-full bg-surface-light dark:bg-background-dark overflow-hidden">
          <div
            ...
            style={{ backgroundImage: "radial-gradient(circle at 10% 20%, rgba(19, 91, 236, 0.1) 0%, transparent 40%), radial-gradient(circle at 90% 80%, rgba(19, 91, 236, 0.1) 0%, transparent 40%)" }}
          />
          <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-10 py-16 md:py-24 flex flex-col items-center justify-center text-center">
            <div className="mb-8 space-y-4 max-w-2xl">
              <h2
                className="text-4xl md:text-5xl font-black text-text-main dark:text-white tracking-tight leading-tight"
                style={{ animation: "slideDown 0.7s cubic-bezier(0.16,1,0.3,1) 0.1s both" }}
              >
                Apa yang bisa kami bantu?
              </h2>
              <p
                className="text-text-sub dark:text-gray-400 text-lg leading-relaxed"
            ...
            {/* Search Bar */}
            <div style={{ animation: "fadeInUp 0.7s cubic-bezier(0.16,1,0.3,1) 0.35s both" }} className="w-full max-w-2xl">
              <form action="/informasi/search" method="get" className="relative group">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {categories && categories.length > 0 ? (
                categories.map((cat, i) => (
                  <AnimateOnScroll key={cat.id} stagger={((i % 6) + 1) as 1|2|3|4|5|6} variant="scale-up">
                    <CategoryCard name={cat.title} slug={cat.slug} description={cat.description} icon={cat.icon} />
                  </AnimateOnScroll>
                ))
              ) : (
